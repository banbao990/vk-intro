...
void RasVertexApp::add_pipeline(const char *vertex_shader_relative_path,
                                const char *fragment_shader_relative_path,
                                VkDescriptorSetLayout *set_layout, uint32_t set_layout_count,
                                bool use_z_buffer, VkRenderPass render_pass, uint32_t subpass,
    ...
    pipeline_builder._vertex_input_info.vertexAttributeDescriptionCount =
        (uint32_t)vertex_desc.attributes.size();
    ...
    pipeline_builder._vertex_input_info.vertexBindingDescriptionCount =
        (uint32_t)vertex_desc.bindings.size();
    // drawing triangle lists, strips, or individual points
    pipeline_builder._input_assembly =
        vkinit::pipeline_input_assembly_create_info(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    ...
    // z-buffer
    if (use_z_buffer) {
        pipeline_builder._depth_stencil =
    ...
    _main_deletion_queue.push_function([=]() { vkDestroyPipeline(_device, pipeline, nullptr); });
...
AllocatedBuffer RasVertexApp::create_buffer(uint32_t alloc_size, VkBufferUsageFlags usage,
    ...
    VK_CHECK(vmaCreateBuffer(_allocator, &buffer_info, &vma_create_info, &buffer._buffer,
...
void RasVertexApp::upload_mesh(Mesh &mesh) {
    ...
    AllocatedBuffer staging_buffer =
        create_buffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
