...
public:
    RasVertexApp(const char *name, uint32_t width, uint32_t height, bool use_validation_layer)
...
protected:
    ...
    void add_pipeline(const char *vertex_shader_relative_path,
                      const char *fragment_shader_relative_path, VkDescriptorSetLayout *set_layout,
                      uint32_t set_layout_count, bool use_z_buffer, VkRenderPass renderpass,
                      uint32_t subpass, VkPipeline &pipeline, VkPipelineLayout &layout);
    AllocatedBuffer create_buffer(uint32_t alloc_size, VkBufferUsageFlags usage,
