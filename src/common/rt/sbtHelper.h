...
public:
    ...
    void initialize(const uint32_t num_hit_groups, const uint32_t num_miss_groups,
                    const uint32_t shader_handle_size, const uint32_t shader_group_alignment);
    void destroy();
    void set_raygen_stage(const VkPipelineShaderStageCreateInfo &stage);
    void add_stage_to_hit_groups(const std::vector<VkPipelineShaderStageCreateInfo> &stages,
                                 const uint32_t group_index);
    void add_stage_to_miss_groups(const VkPipelineShaderStageCreateInfo &stage,
                                  const uint32_t group_index);
