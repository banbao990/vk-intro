...
void LoaderManager::init_extension_addr(const VkDevice device) {
    ...
    instance->vkGetBufferDeviceAddressKHR = (PFN_vkGetBufferDeviceAddressKHR)vkGetDeviceProcAddr(
...
AllocatedBuffer rt_utils::create_buffer(const VmaAllocator &allocator, const uint32_t alloc_size,
    ...
    VK_CHECK(vmaCreateBuffer(allocator, &buffer_info, &vma_create_info, &buffer._buffer,
...
VkDeviceOrHostAddressKHR rt_utils::get_buffer_device_address(const VkDevice &device,
                                                             const VkBuffer &buffer) {
    VkBufferDeviceAddressInfoKHR info = {VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO, nullptr,
                                         buffer};

    VkDeviceOrHostAddressKHR result;
    result.deviceAddress =
        LoaderManager::get_instance()->vkGetBufferDeviceAddressKHR(device, &info);

    return result;
}

VkDeviceOrHostAddressConstKHR rt_utils::get_buffer_device_address_const(const VkDevice &device,
                                                                        const VkBuffer &buffer) {
...
void RTScene::build_blas(VkDevice device, VmaAllocator allocator, RTApp *app) {
    ...
    std::vector<VkAccelerationStructureBuildRangeInfoKHR> ranges(
        num_meshes, VkAccelerationStructureBuildRangeInfoKHR{});
    std::vector<VkAccelerationStructureBuildGeometryInfoKHR> build_infos(
        num_meshes, VkAccelerationStructureBuildGeometryInfoKHR{
                        VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_GEOMETRY_INFO_KHR});
    std::vector<VkAccelerationStructureBuildSizesInfoKHR> size_infos(
        num_meshes, VkAccelerationStructureBuildSizesInfoKHR{
    ...
    for (size_t i = 0; i < num_meshes; ++i) {
        ...
        triangle.vertexData = rt_utils::get_buffer_device_address_const(
            device, mesh._positions._buffer);  // device or host address
        triangle.vertexStride = sizeof(vec3);
        triangle.maxVertex = mesh._num_vertices;
        triangle.indexData =
            rt_utils::get_buffer_device_address_const(device, mesh._indices._buffer);
        ...
        LoaderManager::get_instance()->vkGetAccelerationStructureBuildSizesKHR(
    ...
    AllocatedBuffer scratch_buffer = rt_utils::create_buffer(
        allocator, max_blas_size,
        VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY);

    LoaderManager *loader_manager = LoaderManager::get_instance();

    app->immediate_submit([&](VkCommandBuffer cmd) {
        ...
        for (size_t i = 0; i < num_meshes; ++i) {
            ...
            mesh._blas._buffer =
                rt_utils::create_buffer(allocator, size_info.accelerationStructureSize,
            ...
            loader_manager->vkCreateAccelerationStructureKHR(device, &create_info, nullptr,
                                                             &mesh._blas._acceleration_structure);

            // aligned
            // VkPhysicalDeviceAccelerationStructurePropertiesKHR::minAccelerationStructureScratchOffsetAlignment
            build_info.scratchData =
                rt_utils::get_buffer_device_address(device, scratch_buffer._buffer);
            build_info.scratchData.deviceAddress =
                vkutils::padding(build_info.scratchData.deviceAddress,
                                 app->get_min_acceleration_structure_scratch_offset_alignment());
    ...
    for (size_t i = 0; i < num_meshes; ++i) {
        ...
        mesh._blas._handle =
            loader_manager->vkGetAccelerationStructureDeviceAddressKHR(device, &address_info);
    }
    ...
    app->add_to_deletion_queue([=]() {
        for (size_t i = 0; i < num_meshes; ++i) {
            ...
            loader_manager->vkDestroyAccelerationStructureKHR(
                device, mesh._blas._acceleration_structure, nullptr);
        }
    });
}
...
void RTScene::build_tlas(VkDevice device, VmaAllocator allocator, RTApp *app) {
    ...
    std::vector<VkAccelerationStructureInstanceKHR> instances(num_meshes,
    ...
    AllocatedBuffer staging_buffer = rt_utils::create_buffer(
        allocator, instance_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
    ...
    AllocatedBuffer instance_buffer = rt_utils::create_buffer(
        allocator, instance_size,
    ...
    tlas_instance_info.data =
        rt_utils::get_buffer_device_address_const(device, instance_buffer._buffer);
    ...
    VkAccelerationStructureBuildSizesInfoKHR size_info = {
        VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR};
    loader_manager->vkGetAccelerationStructureBuildSizesKHR(
        device, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &build_info, &num_instances,
        &size_info);
    ...
    VK_CHECK(loader_manager->vkCreateAccelerationStructureKHR(device, &create_info, nullptr,
                                                              &_tlas._acceleration_structure));

    // just for staging, need it to build tlas
    AllocatedBuffer scratch_buffer = rt_utils::create_buffer(
        allocator, size_info.accelerationStructureSize,
        VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY);
    ...
    app->add_to_deletion_queue([=]() {
        ...
        loader_manager->vkDestroyAccelerationStructureKHR(device, _tlas._acceleration_structure,
                                                          nullptr);
    });
}

void rt_utils::image_barrier(VkCommandBuffer cmd, VkImage image,
