...
physical_device_shader_draw_parameters_features(VkBool32 shader_draw_parameters);
...
VkRenderPassBeginInfo renderpass_begin_info(VkRenderPass render_pass, VkExtent2D window_extent,
...
pipeline_input_assembly_create_info(VkPrimitiveTopology topology);

VkPipelineRasterizationStateCreateInfo
pipeline_rasterization_state_create_info(VkPolygonMode polygonMode);

// MSAA
VkPipelineMultisampleStateCreateInfo pipeline_multisampling_state_create_info();

VkPipelineViewportStateCreateInfo pipeline_viewport_create_info(VkViewport *viewport,
                                                                VkRect2D *scissor);
...
VkImageCreateInfo image_create_info(VkFormat format, VkImageUsageFlags usage_flags,
                                    VkExtent3D extent);
...
depth_stencil_create_info(bool b_depth_test, bool b_depth_write, VkCompareOp compare_op);
...
VkDescriptorSetLayoutBinding descriptor_set_layout_binding(VkDescriptorType type,
...
VkWriteDescriptorSet write_descriptor_image(VkDescriptorType type, VkDescriptorSet dst_set,
