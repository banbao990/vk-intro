...
MeshBufferApp::MeshBufferApp(const char *name, uint32_t width, uint32_t height,
                             bool use_validation_layer)
    : RasBufferApp(name, width, height, use_validation_layer) {
...
void MeshBufferApp::init_pipeline() {
    add_pipeline("04-mesh.vert.spv", "04-mesh.frag.spv", _set_layout.data(), _set_layout.size(),
                 true, _render_pass, 0, _mesh_pipeline, _mesh_pipeline_layout);
}
...
void MeshBufferApp::render() {
...

    {
        ...
        glm::mat4 projection =
            glm::perspective(glm::radians(70.0f), 1700.0f / 900.0f, 0.1f, 200.0f);
        projection[1][1] *= -1;
    ...
    const int OBJ_NUM = 3;
    {
        ...
        for (int i = 0; i < OBJ_NUM; ++i) {
            object_ssbo[i]._model_matrix = glm::scale(
                glm::translate(trans, glm::vec3(0.5f * (i * OBJ_NUM / 2), 0, 0)), glm::vec3(3.0f));
        }
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _mesh_pipeline_layout, 0, 1,
                            &frame._uniform_data_descriptor_set, 1, &uniform_offset);

    // (storage)
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _mesh_pipeline_layout, 1, 1,
    ...
    vkCmdPushConstants(cmd, _mesh_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                       sizeof(MeshPushConstant), &mesh_constants);
...
void MeshBufferApp::set_shader_input(PipelineBuilder &builder, VkPipelineLayout &layout,
